...
app.get("/api/proyeccion", (req, res) => {
  ...
  // Promedio de egresos mensuales (últimos 12 meses, sin amortización ni inversores = gasto operativo)
  const hace12 = new Date(); hace12.setMonth(hace12.getMonth() - 12);
  const desde12 = hace12.toISOString().slice(0, 10);
  const eg = db.prepare(`SELECT COALESCE(SUM(egreso),0) t FROM transacciones
    WHERE fecha >= ? AND egreso > 0 AND fuente NOT IN ('amortizacion','inversor')`).get(desde12);
  const promedioEgreso = Math.round((eg.t / 12) * 100) / 100;
  ...
  for (let i = 0; i < meses; i++) {
    ...
    // Inversores que vencen ese mes (capital + interés al vencimiento)
    ...
    const egresoEst = (aj.egreso_estimado != null) ? aj.egreso_estimado : promedioEgreso;
    ...
    proyeccion.push({
      ...
      saldo_final: saldo, editado: !!ajustes[mes]
    });
  }
  res.json({ caja_inicial: Math.round((c.ing - c.egr) * 100) / 100, promedio_egreso: promedioEgreso, proyeccion });
});
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.23.0-ndvi-ranking" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.23.0-ndvi-ranking corriendo en puerto ${PORT}`);
