...
app.post("/api/ordenes", (req, res) => {
  const { lote, titulo, anio, ciclo, numero, notas, hectareas } = req.body;
  const anioFinal = parseInt(anio) || new Date().getFullYear();
  const numFinal = numero ? parseInt(numero) : getNextNumeroOrden(anioFinal);
  const loteUp = lote ? lote.toUpperCase() : '';
  // ha: si no vienen y hay lote, tomo las ha sembrables del lote
  let ha = parseFloat(hectareas);
  if ((isNaN(ha) || ha === 0) && loteUp) {
    const lt = db.prepare("SELECT ha_sembrables FROM lotes WHERE nombre = ?").get(loteUp);
  ...
  const r = db.prepare("INSERT INTO ordenes_trabajo (numero,anio,lote,titulo,ciclo,hectareas,estado,notas) VALUES (?,?,?,?,?,?,'PLANIFICADA',?)")
    .run(numFinal, anioFinal, loteUp, titulo||'', ciclo||'', ha||0, notas||'');
  res.json({ id: r.lastInsertRowid, numero: numFinal, anio: anioFinal, hectareas: ha||0, numero_display: `${numFinal}/${anioFinal}` });
});

app.put("/api/ordenes/:id", (req, res) => {
  const { lote, titulo, ciclo, estado, notas, hectareas } = req.body;
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.3.2-dosis" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.3.2-dosis corriendo en puerto ${PORT}`);
