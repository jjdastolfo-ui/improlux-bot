...
app.post("/api/bienes/amortizar", (req, res) => {
  ...
  let creados = 0, total = 0, saltados = 0;
  const insT = db.prepare(`INSERT INTO transacciones (fecha, concepto, detalle, ingreso, egreso, proveedor, tc, fuente)
    VALUES (?, 'AMORTIZACION MAQUINARIA', ?, 0, ?, '', 0, 'amortizacion')`);
  ...
  for (const b of bienes) {
    ...
    const t = insT.run(fecha, `Depreciación ${b.nombre} · ciclo ${cicloObj.ciclo}`, b.amort_anual);
    insA.run(b.id, cicloObj.ciclo, b.amort_anual, t.lastInsertRowid, fecha);
    creados++; total += b.amort_anual;
  }
  res.json({ ok: true, ciclo: cicloObj.ciclo, creados, total: Math.round(total * 100) / 100, ya_existentes: saltados });
});
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.6.2-bienes-devengado" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.6.2-bienes-devengado corriendo en puerto ${PORT}`);
