...
function getStockValorizado() {
  const productos = db.prepare("SELECT * FROM stock_productos ORDER BY rubro, categoria, nombre").all();
...
app.post("/api/stock", (req, res) => {
  const { nombre, rubro, categoria, unidad, cantidad, precio_unitario, notas } = req.body;
  if (!nombre) return res.status(400).json({ error: 'Falta nombre' });
  try {
    const r = db.prepare("INSERT INTO stock_productos (nombre,rubro,categoria,unidad,cantidad,precio_unitario,notas) VALUES (?,?,?,?,?,?,?)")
      .run(nombre.toUpperCase(), (rubro||'AGRICOLA').toUpperCase(), (categoria||'OTRO').toUpperCase(), unidad||'kg', parseFloat(cantidad)||0, parseFloat(precio_unitario)||0, notas||'');
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.10.0-insumos-rubro" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.10.0-insumos-rubro corriendo en puerto ${PORT}`);
