...
app.get("/api/ordenes", (req, res) => {
  ...
  let q = "SELECT o.*, lt.hectareas, lt.ha_sembrables FROM ordenes_trabajo o LEFT JOIN lotes lt ON lt.nombre = o.lote WHERE 1=1";
  const p = [];
...
app.post("/api/ordenes", (req, res) => {
  const { lote, titulo, anio, ciclo, numero, notas } = req.body;
  const anioFinal = parseInt(anio) || new Date().getFullYear();
  const numFinal = numero ? parseInt(numero) : getNextNumeroOrden(anioFinal);
  const r = db.prepare("INSERT INTO ordenes_trabajo (numero,anio,lote,titulo,ciclo,estado,notas) VALUES (?,?,?,?,?,'PLANIFICADA',?)")
    .run(numFinal, anioFinal, lote ? lote.toUpperCase() : '', titulo||'', ciclo||'', notas||'');
  res.json({ id: r.lastInsertRowid, numero: numFinal, anio: anioFinal, numero_display: `${numFinal}/${anioFinal}` });
});

app.put("/api/ordenes/:id", (req, res) => {
  const { lote, titulo, ciclo, estado, notas } = req.body;
...
app.post("/api/ordenes/:id/items", (req, res) => {
  const { tipo, etapa, producto_id, descripcion, cantidad, unidad, precio_unitario } = req.body;
  const o = db.prepare("SELECT * FROM ordenes_trabajo WHERE id=?").get(req.params.id);
  if (!o) return res.status(404).json({ error: 'Orden no encontrada' });
  ...
  const r = db.prepare("INSERT INTO orden_items (orden_id,tipo,etapa,producto_id,descripcion,cantidad,unidad,precio_unitario,total) VALUES (?,?,?,?,?,?,?,?,?)")
    .run(req.params.id, tipoFinal, etapaFinal, tipoFinal==='INSUMO'?prodId:null, desc, cant, uni||'kg', precio, total);
  const result = recalcularOrden(req.params.id);
  res.json({ id: r.lastInsertRowid, total, ...result });
});
...
app.put("/api/ordenes/:id/items/:itemId", (req, res) => {
  ...
  const nuevaCant = cantidad !== undefined ? (parseFloat(cantidad)||0) : item.cantidad;
  ...
  db.prepare("UPDATE orden_items SET tipo=?,etapa=?,producto_id=?,descripcion=?,cantidad=?,unidad=?,precio_unitario=?,total=? WHERE id=?")
    .run(nuevoTipo, nuevaEtapa, nuevoTipo==='INSUMO'?nuevoProd:null, nuevaDesc, nuevaCant, nuevaUni, nuevoPrecio, total, req.params.itemId);
  const result = recalcularOrden(req.params.id);
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.3" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.3 corriendo en puerto ${PORT}`);
