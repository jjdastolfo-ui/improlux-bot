...
// Permite dejar el stock en negativo, pero lo avisa en la respuesta.
app.post("/api/stock/aplicar", (req, res) => {
  ...
  // Ubicar el producto: por id, o por nombre (+ campo si viene)
  let prod = null;
  if (producto_id) prod = db.prepare("SELECT * FROM stock_productos WHERE id = ?").get(producto_id);
  if (!prod && producto) {
    prod = campo
      ? db.prepare("SELECT * FROM stock_productos WHERE LOWER(nombre)=LOWER(?) AND campo=?").get(producto, campo)
      : db.prepare("SELECT * FROM stock_productos WHERE LOWER(nombre)=LOWER(?)").get(producto);
  }
  if (!prod) return res.status(404).json({ error: "No encontré ese producto en el stock" });
  ...
  res.json({
    ...
    negativo,
    aviso: negativo ? `⚠️ Stock de ${prod.nombre} quedó en negativo (${Math.round(nuevaCantidad*100)/100} ${prod.unidad}). Registrá una compra para reponer.` : null
  });
});
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.13.0-aplicar-stock" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.13.0-aplicar-stock corriendo en puerto ${PORT}`);
