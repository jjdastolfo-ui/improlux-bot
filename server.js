...
function getLaboreoDetalle(laboreoId) {
  ...
  if (!lab) return null;
  const items = db.prepare("SELECT * FROM laboreo_items WHERE laboreo_id = ? ORDER BY categoria, id").all(laboreoId);
  const lote = db.prepare("SELECT * FROM lotes WHERE nombre = ?").get(lab.lote);
  return { ...lab, items, lote };
}

function recalcularTotalLaboreo(laboreoId) {
  const total = calcularTotalLaboreo(laboreoId);
  db.prepare("UPDATE laboreos SET total_presupuestado = ? WHERE id = ?").run(total, laboreoId);
  return total;
}
...
app.put("/api/laboreos/:id/items/:itemId", (req, res) => {
  const { descripcion, cantidad, unidad, precio_unitario, categoria } = req.body;
  const total = (parseFloat(cantidad)||0) * (parseFloat(precio_unitario)||0);
  db.prepare("UPDATE laboreo_items SET descripcion=?,cantidad=?,unidad=?,precio_unitario=?,total=?,categoria=? WHERE id=? AND laboreo_id=?").run(descripcion, parseFloat(cantidad)||0, unidad||'kg', parseFloat(precio_unitario)||0, total, categoria||'INSUMO', req.params.itemId, req.params.id);
  const tot = db.prepare("SELECT COALESCE(SUM(total),0) as t FROM laboreo_items WHERE laboreo_id=?").get(req.params.id).t;
  db.prepare("UPDATE laboreos SET total_presupuestado=? WHERE id=?").run(tot, req.params.id);
