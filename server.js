...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS extractos_bancarios (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    banco TEXT NOT NULL,
    fecha_extracto TEXT NOT NULL,
    fecha_desde TEXT,
    fecha_hasta TEXT,
    movimientos TEXT DEFAULT '[]',
...
function getSaldoProveedor(proveedor) {
  const row = db.prepare(`
    SELECT 
      COALESCE(SUM(CASE WHEN concepto != 'PAGOS CUENTA CORRIENTE' THEN egreso ELSE 0 END), 0) as total_compras,
      COALESCE(SUM(CASE WHEN concepto = 'PAGOS CUENTA CORRIENTE' THEN egreso ELSE 0 END), 0) as total_pagos
    FROM transacciones 
...
async function ejecutarAccion(accion) {
  ...
  if (accion.accion === "pago_proveedor") {
    ...
    db.prepare(`
      INSERT INTO transacciones (fecha, concepto, detalle, ingreso, egreso, proveedor, tc, fuente)
      VALUES (?, 'PAGOS CUENTA CORRIENTE', ?, 0, ?, ?, ?, 'whatsapp')
...
// ── PROCESAR FACTURA POR IMAGEN ───────────────────────────────────────────────
async function procesarFactura(mediaUrl, mediaType) {
  try {
    ...
    const result = await anthropic.messages.create({
      ...
      messages: [{ role: "user", content: [
        ...
        { type: "text", text: `Analizá esta factura/ticket de una empresa ganadera uruguaya.
Extraé TODOS los ítems con su categoría correspondiente de esta lista: ${CATEGORIAS.join(", ")}.
Si hay múltiples ítems de diferentes categorías, listá cada uno por separado.
Respondé SOLO con JSON array:
[{"descripcion":"...","monto":0.00,"moneda":"USD","concepto":"CATEGORIA","nota":"..."}]
Si es un solo ítem, igual devolvé array con un elemento.` }
      ]}]
    });
    ...
    for (const item of items) {
      ...
      let nota = item.nota || "";
      if ((item.moneda || "USD").toUpperCase() === "UYU" && tc) {
        nota = `$${monto.toFixed(2)} UYU × TC ${tc.toFixed(2)}${nota ? " | " + nota : ""}`;
        monto = monto / tc;
      }
      db.prepare(`INSERT INTO transacciones (fecha, concepto, detalle, ingreso, egreso, proveedor, tc, fuente) VALUES (?, ?, ?, 0, ?, '', ?, 'whatsapp')`).run(hoy, item.concepto, item.descripcion, monto, tc || 0);
      respuestas.push(`✅ ${item.descripcion} · $${monto.toFixed(2)} USD · ${item.concepto}`);
    }

    return `📸 *Factura procesada — ${items.length} ítem(s):*\n\n${respuestas.join("\n")}`;
  } catch(e) {
    console.error("Error factura:", e);
    return "❌ No pude leer la factura. Registrá manualmente.";
  }
}
...
function iniciarScheduler() {
  setInterval(async () => {
    ...
    const hora = horaUY.getUTCHours();
    const minuto = horaUY.getUTCMinutes();
    const diaSemana = horaUY.getUTCDay();
    if (minuto !== 0) return;
    ...
    if (diaSemana === 1 && hora === 8) {
      ...
      const msg = `📊 *Resumen semanal IMPROLUX*\n\n💰 Egresos del mes: $${fmt(egresosMes?.total||0)} USD\n🔄 Deuda cuentas corrientes: $${fmt(totalCuentas)} USD\n🏦 Cheques pendientes: ${chequesPend?.total||0} ($${fmt(chequesPend?.monto||0)} USD)`;
      try {
        await twilioClient.messages.create({ from: `whatsapp:${TWILIO_NUMBER}`, to: NUMERO_ADMIN, body: msg });
      } catch(e) { console.error("Error scheduler:", e.message); }
    }

    // Diario 9am — cheques que vencen en 7 días
    if (hora === 9) {
      const en7dias = new Date(Date.now() + 7*24*60*60*1000).toISOString().slice(0,10);
      const hoy = new Date().toISOString().slice(0,10);
      const cheques = db.prepare(`SELECT * FROM cheques WHERE estado = 'PENDIENTE' AND fecha_cobro BETWEEN ? AND ?`).all(hoy, en7dias);
      if (cheques.length > 0) {
        const lineas = cheques.map(c=>`  ${c.tipo==='EMITIDO'?'📤':'📥'} ${c.proveedor||'Sin prov'} · $${fmt(c.monto)} · vence ${c.fecha_cobro}`).join('\n');
        const msg = `⚠️ *IMPROLUX — Cheques próximos a vencer:*\n\n${lineas}`;
        try {
          await twilioClient.messages.create({ from: `whatsapp:${TWILIO_NUMBER}`, to: NUMERO_ADMIN, body: msg });
        } catch(e) { console.error("Error scheduler cheques:", e.message); }
      }
    }
  }, 60*1000);
  console.log("⏰ Scheduler IMPROLUX iniciado");
}
...
app.post("/webhook-interno", async (req, res) => {
  try {
    ...
    if (numMedia > 0 && mediaType.startsWith("image/") && mediaUrl) {
      respuesta = await procesarFactura(mediaUrl, mediaType);
      res.json({ respuesta });
      return;
    }
...
app.post("/api/importar-cheques", (req, res) => {
  const { cheques } = req.body;
  if (!Array.isArray(cheques)) return res.status(400).json({ error: 'Formato inválido' });
  let ok = 0, errores = 0;
  const stmt = db.prepare(`
    INSERT INTO cheques (fecha_emision, fecha_cobro, tipo, proveedor, monto, estado, banco, concepto)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  ...
  for (const c of cheques) {
    try {
      stmt.run(c.fecha_emision||'', c.fecha_cobro||'', c.tipo||'EMITIDO', c.proveedor||'', parseFloat(c.monto)||0, c.estado||'PENDIENTE', c.banco||'BROU', c.concepto||'');
      ok++;
    } catch(e) { errores++; }
  }
...
app.post("/api/extracto", async (req, res) => {
  try {
    const { banco, base64 } = req.body;
    if (!base64 || !banco) return res.status(400).json({ error: 'Faltan datos' });
    ...
    const result = await anthropic.messages.create({
      ...
      messages: [{
        ...
        content: [
          ...
          { type: "text", text: `Analizá este extracto bancario del banco ${banco} de Uruguay.
Extraé TODOS los movimientos. Respondé SOLO con JSON sin markdown:
{
  "banco": "${banco}",
  ...
  "saldo_final": 0,
  "movimientos": [
    {"fecha": "YYYY-MM-DD", "descripcion": "...", "monto": 0, "tipo": "CREDITO o DEBITO"}
  ]
}` }
        ]
      }]
    ...
    const raw = result.content[0].text.trim().replace(/```json|```/g, "").trim();
    const extracto = JSON.parse(raw);

    // Guardar extracto
    const r = db.prepare(`
      INSERT INTO extractos_bancarios (banco, fecha_extracto, fecha_desde, fecha_hasta, movimientos, saldo_inicial, saldo_final)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(banco, new Date().toISOString().slice(0,10), extracto.fecha_desde||'', extracto.fecha_hasta||'', JSON.stringify(extracto.movimientos||[]), extracto.saldo_inicial||0, extracto.saldo_final||0);

    // Auto-conciliar
    const movimientos = extracto.movimientos || [];
    const conciliacion = movimientos.map(mov => {
      const monto = Math.abs(mov.monto);
      const fecha = mov.fecha;
      const fechaD = new Date(fecha);
      const f1 = new Date(fechaD - 3*86400000).toISOString().slice(0,10);
      const f2 = new Date(fechaD.getTime() + 3*86400000).toISOString().slice(0,10);
      const campo = mov.tipo === 'DEBITO' ? 'egreso' : 'ingreso';
      const coincidencia = db.prepare(`
        SELECT * FROM transacciones
        WHERE fecha BETWEEN ? AND ?
        AND ${campo} BETWEEN ? AND ?
        ORDER BY ABS(${campo} - ?) ASC LIMIT 1
      `).get(f1, f2, monto*0.93, monto*1.07, monto);
      return { mov_banco: mov, coincidencia: coincidencia||null, estado: coincidencia ? 'CONCILIADO' : 'PENDIENTE' };
    });

    res.json({ ok: true, extracto, conciliacion });
  } catch(e) {
    console.error("Error extracto:", e);
    res.status(500).json({ error: e.message });
  }
});

app.get("/api/extractos", (req, res) => {
  const rows = db.prepare("SELECT * FROM extractos_bancarios ORDER BY created_at DESC LIMIT 20").all();
  res.json(rows);
});
