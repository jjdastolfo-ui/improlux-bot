...
app.post("/api/ordenes/:id/items", (req, res) => {
  const { tipo, producto_id, descripcion, cantidad, unidad, precio_unitario } = req.body;
  const o = db.prepare("SELECT * FROM ordenes_trabajo WHERE id=?").get(req.params.id);
  if (!o) return res.status(404).json({ error: 'Orden no encontrada' });
  ...
  const r = db.prepare("INSERT INTO orden_items (orden_id,tipo,producto_id,descripcion,cantidad,unidad,precio_unitario,total) VALUES (?,?,?,?,?,?,?,?)")
    .run(req.params.id, tipoFinal, tipoFinal==='INSUMO'?prodId:null, desc, cant, uni||'kg', precio, total);
...
app.put("/api/ordenes/:id/items/:itemId", (req, res) => {
  ...
  db.prepare("UPDATE orden_items SET tipo=?,producto_id=?,descripcion=?,cantidad=?,unidad=?,precio_unitario=?,total=? WHERE id=?")
    .run(nuevoTipo, nuevoTipo==='INSUMO'?nuevoProd:null, nuevaDesc, nuevaCant, nuevaUni, nuevoPrecio, total, req.params.itemId);
  const result = recalcularOrden(req.params.id);
