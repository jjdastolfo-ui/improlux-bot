  ...
  try {
    ...
    // ── GASTOS POR CATEGORÍA (MES A MES) — página apaisada ──
    {
      ...
      if (cats.length && mesesCols.length) {
        ...
        if (ly > 545) { doc.addPage({ size: 'A4', layout: 'landscape', margin: 40 }); ly = 45; drawHeader(); }
        doc.rect(L, ly, doc.page.width - 2 * L, 15).fill(colorVerde);
        doc.fontSize(6.5).fill('#ffffff').font('Helvetica-Bold').text('TOTAL', L + 4, ly + 4, { width: catW - 6, lineBreak: false });
        let granTot = 0;
        doc.fontSize(6);
        mesesCols.forEach((mc, i) => {
          const colTot = cats.reduce((s, cat) => s + ((matriz[cat] && matriz[cat][mc.periodo]) || 0), 0);
          granTot += colTot;
          doc.fill('#ffffff').text(fmtInt(colTot), monthsX + i * colW, ly + 4.5, { width: colW - 2, align: 'right', lineBreak: false });
        });
        doc.fill('#ffffff').text(fmtInt(granTot), totX, ly + 4.5, { width: totW - 4, align: 'right', lineBreak: false });
      }
    }
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.3.7-mesames" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.3.7-mesames corriendo en puerto ${PORT}`);
