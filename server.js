...
function getStockValorizado() {
  const productos = db.prepare("SELECT * FROM stock_productos ORDER BY categoria, nombre").all();
  return productos.map(p => ({ ...p, valor: (p.cantidad || 0) * (p.precio_unitario || 0) }));
}
...
app.get("/api/bienes", (req, res) => {
  const rows = db.prepare("SELECT * FROM bienes_muebles WHERE activo = 1 ORDER BY valor_compra DESC").all();
...
app.post("/api/bienes", (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO bienes_muebles (nombre, categoria, valor_compra, fecha_compra, vida_util_anios, valor_residual, notas)
    VALUES (?, ?, ?, ?, ?, ?, ?)`).run(nombre, categoria || '', parseFloat(valor_compra), fecha_compra, parseFloat(vida_util_anios) || 10, parseFloat(valor_residual) || 0, notas || '');
...
app.get("/api/ganado", (req, res) => {
  ...
  const igu = getIGU();
  const rows = campo
    ? db.prepare("SELECT * FROM stock_ganadero WHERE campo = ? ORDER BY categoria, registro, id").all(campo)
    : db.prepare("SELECT * FROM stock_ganadero ORDER BY categoria, registro, id").all();
  const conValor = rows.map(r => {
    const kgTot = (r.cantidad || 0) * (r.kg_estimado || 0);
    // Valuación: kg × IGU si hay kg cargados; si no, cae al $/cabeza (modo anterior)
    const valor = (r.kg_estimado > 0 && igu.precio > 0)
      ? Math.round(kgTot * igu.precio * 100) / 100
      : Math.round((r.cantidad || 0) * (r.valor_cabeza || 0) * 100) / 100;
    return { ...r, kg_totales: Math.round(kgTot), valor, valuado_por: (r.kg_estimado > 0 && igu.precio > 0) ? 'igu' : 'cabeza' };
  });
...
app.post("/api/ganado", (req, res) => {
  const { campo, categoria, registro, cantidad, kg_estimado, valor_cabeza, orden, notas } = req.body;
  if (!categoria) return res.status(400).json({ error: "Falta la categoría" });
  const r = db.prepare(`INSERT INTO stock_ganadero (campo, categoria, registro, cantidad, kg_estimado, valor_cabeza, orden, notas)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(campo || 'LA AMISTAD', categoria, (registro || 'GENERAL').toUpperCase(),
      parseFloat(cantidad) || 0, parseFloat(kg_estimado) || 0, parseFloat(valor_cabeza) || 0, parseInt(orden) || 0, notas || '');
  res.json({ id: r.lastInsertRowid });
});
...
app.put("/api/ganado/:id", (req, res) => {
  ...
  const { categoria, registro, cantidad, kg_estimado, valor_cabeza, orden, notas } = req.body;
  db.prepare(`UPDATE stock_ganadero SET categoria=?, registro=?, cantidad=?, kg_estimado=?, valor_cabeza=?, orden=?, notas=?, updated_at=datetime('now') WHERE id=?`)
...
app.post("/api/ganado/sync-ade", async (req, res) => {
  ...
  try {
    ...
    const upd = db.prepare("UPDATE stock_ganadero SET cantidad = ?, origen = 'ade', updated_at = datetime('now') WHERE id = ?");
    const ins = db.prepare("INSERT INTO stock_ganadero (campo, categoria, registro, cantidad, kg_estimado, valor_cabeza, origen) VALUES ('LA AMISTAD', ?, ?, ?, 0, 0, 'ade')");
    ...
    for (const it of lista) {
      ...
      const cant = parseFloat(it.cantidad ?? it.count ?? it.total ?? it.cabezas ?? 0) || 0;
      if (!cat) continue;
      totalCab += cant;
      const ex = getByCatReg.get(cat, reg);
      if (ex) { upd.run(cant, ex.id); actualizados++; }
      else { ins.run(cat, reg, cant); creados++; }
    }
...
app.get("/api/stock", (req, res) => {
  res.json(getStockValorizado());
});

app.post("/api/stock", (req, res) => {
  const { nombre, categoria, unidad, cantidad, precio_unitario, notas } = req.body;
  if (!nombre) return res.status(400).json({ error: 'Falta nombre' });
  try {
    const r = db.prepare("INSERT INTO stock_productos (nombre,categoria,unidad,cantidad,precio_unitario,notas) VALUES (?,?,?,?,?,?)")
      .run(nombre.toUpperCase(), (categoria||'OTRO').toUpperCase(), unidad||'kg', parseFloat(cantidad)||0, parseFloat(precio_unitario)||0, notas||'');
...
app.put("/api/stock/:id", (req, res) => {
  const { nombre, categoria, unidad, notas } = req.body;
  const p = db.prepare("SELECT * FROM stock_productos WHERE id=?").get(req.params.id);
  if (!p) return res.status(404).json({ error: 'Producto no encontrado' });
  db.prepare("UPDATE stock_productos SET nombre=?,categoria=?,unidad=?,notas=? WHERE id=?")
    .run((nombre||p.nombre).toUpperCase(), (categoria||p.categoria).toUpperCase(), unidad||p.unidad, notas!==undefined?notas:p.notas, req.params.id);
  res.json({ ok: true });
});
...
app.get("/api/ordenes", (req, res) => {
  const { anio, lote, estado } = req.query;
...
app.post("/api/ordenes", (req, res) => {
  ...
  ha = ha || 0;
  const r = db.prepare("INSERT INTO ordenes_trabajo (numero,anio,lote,titulo,ciclo,hectareas,estado,notas) VALUES (?,?,?,?,?,?,'PLANIFICADA',?)")
    .run(numFinal, anioFinal, loteStr, titulo||'', ciclo||'', ha, notas||'');
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.9.0-ganado-igu" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.9.0-ganado-igu corriendo en puerto ${PORT}`);
