...
// ── STOCK GANADERO (hacienda valuada por cabeza) ──────────────────────────────
app.get("/api/ganado", (req, res) => {
  const campo = req.query.campo;
  const rows = campo
    ? db.prepare("SELECT * FROM stock_ganadero WHERE campo = ? ORDER BY orden, id").all(campo)
    : db.prepare("SELECT * FROM stock_ganadero ORDER BY orden, id").all();
  const conValor = rows.map(r => ({ ...r, valor: Math.round((r.cantidad || 0) * (r.valor_cabeza || 0) * 100) / 100 }));
  res.json(conValor);
});

app.post("/api/ganado", (req, res) => {
  const { campo, categoria, cantidad, valor_cabeza, orden, notas } = req.body;
  if (!categoria) return res.status(400).json({ error: "Falta la categoría" });
  const r = db.prepare(`INSERT INTO stock_ganadero (campo, categoria, cantidad, valor_cabeza, orden, notas)
    VALUES (?, ?, ?, ?, ?, ?)`).run(campo || 'LA AMISTAD', categoria, parseFloat(cantidad) || 0, parseFloat(valor_cabeza) || 0, parseInt(orden) || 0, notas || '');
  res.json({ id: r.lastInsertRowid });
});
...
app.put("/api/ganado/:id", (req, res) => {
  ...
  const { categoria, cantidad, valor_cabeza, orden, notas } = req.body;
  db.prepare(`UPDATE stock_ganadero SET categoria=?, cantidad=?, valor_cabeza=?, orden=?, notas=?, updated_at=datetime('now') WHERE id=?`)
    .run(categoria ?? g.categoria, cantidad !== undefined ? parseFloat(cantidad) : g.cantidad,
      valor_cabeza !== undefined ? parseFloat(valor_cabeza) : g.valor_cabeza, orden !== undefined ? parseInt(orden) : g.orden,
      notas ?? g.notas, req.params.id);
  res.json({ ok: true });
});
...
app.post("/api/ganado/sync-ade", async (req, res) => {
  ...
  try {
    ...
    const getByCat = db.prepare("SELECT * FROM stock_ganadero WHERE LOWER(categoria) = LOWER(?)");
    const upd = db.prepare("UPDATE stock_ganadero SET cantidad = ?, origen = 'ade', updated_at = datetime('now') WHERE id = ?");
    const ins = db.prepare("INSERT INTO stock_ganadero (campo, categoria, cantidad, valor_cabeza, origen) VALUES ('LA AMISTAD', ?, ?, 0, 'ade')");
    ...
    for (const it of lista) {
      ...
      if (!cat) continue;
      totalCab += cant;
      const ex = getByCat.get(cat);
      if (ex) { upd.run(cant, ex.id); actualizados++; }
      else { ins.run(cat, cant); creados++; }
    }
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.8.1-ganado-ade" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.8.1-ganado-ade corriendo en puerto ${PORT}`);
