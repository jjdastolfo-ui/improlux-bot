...
async function buildContexto() {
  ...
  if (inf) {
    db.prepare(`
      SELECT concepto, substr(fecha,1,7) as mes, SUM(egreso) as eg
      FROM transacciones WHERE fecha >= ? AND fecha <= ? AND egreso > 0
      GROUP BY concepto, mes
    `).all(inf.ciclo.fecha_desde, inf.fechaHasta)
      .forEach(r => { (catMes[r.concepto] = catMes[r.concepto] || {})[r.mes] = Math.round(r.eg); });
  }
...
DATOS DEL DASHBOARD (reales — usalos con precisión para responder consultas):
...
INGRESOS DEL CICLO POR CATEGORÍA: ${JSON.stringify(inf.rows.filter(r => r.total_ingreso > 0).map(r => ({ cat: r.concepto, ingreso: Math.round(r.total_ingreso) })))}
GASTOS POR CATEGORÍA Y MES DEL CICLO (para "cuánto gasté en X en tal mes"): ${JSON.stringify(catMes)}` : ''}
...
Egresos del mes actual por categoría: ${JSON.stringify(egresosMes)}
...
- Si el usuario PREGUNTA por datos (cuánto gasté en X, neto de un mes, mis mayores gastos, saldos, comparaciones entre meses, total del ciclo, etc.): respondé DIRECTO, PRECISO y NATURAL usando los datos de arriba. NO devuelvas JSON para preguntas.
- Calculá vos las sumas/restas/rankings que hagan falta con esos datos. Si el dato exacto no está en el contexto, decilo con franqueza y ofrecé lo más cercano que sí tengas.
- Los meses van en formato "YYYY-MM"; "octubre" = el 10, etc. El ciclo va de marzo a febrero.
- Usá JSON SOLO para acciones que CREAN o MODIFICAN datos (registrar, pagar, borrar, editar, informe/pdf, backup).
- Sé conversacional y útil, como un buen contador que conoce el campo.`;
}
...
app.get("/api/informe-pdf", async (req, res) => {
  try {
    ...
    // ── GASTOS POR CATEGORÍA (MES A MES) — página apaisada ──
    {
      ...
      for (let m = 3; m <= 14; m++) {
        const mesReal = m <= 12 ? m : m - 12;
        const anioReal = m <= 12 ? anioIni : anioIni + 1;
        const periodo = `${anioReal}-${String(mesReal).padStart(2, '0')}`;
      ...
      if (cats.length && mesesCols.length) {
        ...
        cats.forEach((cat, idx) => {
          ...
          mesesCols.forEach((mc, i) => {
            ...
            rowTot += v;
            if (v > 0) doc.fill('#c0392b').text(fmtInt(v), monthsX + i * colW, ly + 4, { width: colW - 2, align: 'right', lineBreak: false });
          });
        ...
        const drawResumen = (label, valorFn, granTotFn, opts) => {
          ...
          mesesCols.forEach((mc, i) => {
            ...
            gt += v;
            const col = opts.signColor ? (v >= 0 ? '#1e7d34' : '#c0392b') : (opts.fg || '#ffffff');
          ...
          doc.fill(opts.signColor ? (gtVal >= 0 ? '#1e7d34' : '#c0392b') : (opts.fg || '#ffffff'))
        ...
        drawResumen('INGRESOS', p => ingMes[p] || 0, () => informe.totalIngresos, { bg: '#e8f3e8', fg: '#1e7d34' });
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.4.0-bot-sonnet" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.4.0-bot-sonnet corriendo en puerto ${PORT}`);
