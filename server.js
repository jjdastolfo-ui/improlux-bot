...
function getPatrimonioActual() {
  // Caja: saldo real acumulado de todas las transacciones (finanzas globales)
  const c = db.prepare("SELECT COALESCE(SUM(ingreso),0) ing, COALESCE(SUM(egreso),0) egr FROM transacciones").get();
  ...
  // Ganado: valor del plantel (kg × IGU, o $/cabeza si no hay kg)
  ...
  const ganado = Math.round(db.prepare("SELECT * FROM stock_ganadero").all().reduce((s, r) => {
    ...
    const v = (kg > 0 && igu > 0) ? (r.cantidad || 0) * kg * igu : (r.cantidad || 0) * (r.valor_cabeza || 0);
    return s + v;
  }, 0) * 100) / 100;
  ...
  const total = Math.round((caja + fondo + ganado + bienes + stock) * 100) / 100;
  return { caja, fondo, ganado, bienes, stock, total };
}
...
app.post("/api/patrimonio/snapshot", (req, res) => {
  ...
  if (fecha.endsWith('-02-29')) fecha = fecha.replace('-02-29', '-02-28');
  db.prepare(`INSERT INTO patrimonio_snapshots (ciclo, fecha, caja, ganado, bienes, stock, total, fondo, notas)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(ciclo) DO UPDATE SET fecha=excluded.fecha, caja=excluded.caja, ganado=excluded.ganado,
      bienes=excluded.bienes, stock=excluded.stock, total=excluded.total, fondo=excluded.fondo, created_at=datetime('now')`)
    .run(ciclo, fecha, p.caja, p.ganado, p.bienes, p.stock, p.total, p.fondo, req.body.notas || '');
...
app.get("/", (req, res) => res.json({ status: "IMPROLUX Bot activo 🟢", version: "4.16.1-fondo" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`IMPROLUX Bot v4.16.1-fondo corriendo en puerto ${PORT}`);
